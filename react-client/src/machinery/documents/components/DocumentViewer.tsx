  ...
  function breadcrumbNavigate(e: React.MouseEvent<HTMLAnchorElement, MouseEvent>, to: string, isCurrent: boolean) {
    ...
    switch (to) {
      ...
      case "machinery-documents": {
        navigate(`/machinery/${machinery.uid}/documents`, {
          state: { machinery: machinery, companyName: location.state.companyName },
        });
        break;
      }
      case "company-list": {
      ...
      case "machineries-list": {
        navigate("/companies/" + machinery.companyID, {
          state: "machinery_management",
        });
        break;
      }
  ...
  const memoizedPDF = useMemo(
    () => (
      <Document
        ...
        loading={
          <HStack w="full" h="250px" alignItems="center" justifyContent="center">
            <Spinner size="xl" />
          </HStack>
        }
      >
  ...
  return (
    <Box ref={wrapperRef} w="full" textAlign="left">
      {Number(principal?.companyID) === 0 ? (
        <Breadcrumb mb={2}>
          <BreadcrumbItem>
            <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "company-list", false)}>Company List</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbItem>
            <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "machineries-list", false)}>
              {location.state.companyName}{" "}
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbItem>
            <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "machinery-documents", false)}>
              {machinery.uid}
            </BreadcrumbLink>
          </BreadcrumbItem>
        ...
        <Breadcrumb mb={2}>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "machinery-landing", false)}>
              {machinery.uid}
            </BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbItem>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "", true)}>{document.name}</BreadcrumbLink>
          </BreadcrumbItem>
        </Breadcrumb>
      )}
      ...
      {documentData != null && (
        <Box w="full" textAlign="center">
          ...
          >
            <HStack>
              <InputGroup size="md" w="225px">
                ...
                <Input
                  ...
                  max={numPages || 1}
                />
                <InputRightAddon>out of {numPages}</InputRightAddon>
              </InputGroup>
            </HStack>
            ...
            <Divider orientation="vertical" h="auto" />
            <Button variant="unstyled" leftIcon={<FiDownload />} onClick={downloadDocument}>
              Download document
            </Button>
          </HStack>
          ...
          >
            <Button variant="unstyled" isDisabled={pageNumber <= 1} onClick={previousPage}>
              Previous page
            </Button>
            <Text fontSize="md" fontWeight={500}>
              Page {pageNumber || (numPages ? 1 : "--")} of {numPages || "--"}
            </Text>
            <Button variant="unstyled" isDisabled={pageNumber >= (numPages || 0)} onClick={nextPage}>
              Next page
            </Button>
          </HStack>
        </Box>
