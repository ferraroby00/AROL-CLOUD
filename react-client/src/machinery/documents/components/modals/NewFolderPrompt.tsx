...
import React, { type ChangeEvent, useEffect, useRef, useState, useContext } from "react";
...
export default function NewFolderPrompt(props: NewFolderPromptProps) {
    ...
    async function create() {
      ...
      try {
        const folder: FileMapEntry = {
          ...
          size: 0,
          isPrivate: Number(principal?.companyID) !== 0,
  ...
  return (
    <AlertDialog isOpen={newFolderPromptOpen} leastDestructiveRef={cancelRef} onClose={handleCancel}>
      <AlertDialogOverlay>
        <AlertDialogContent>
          ...
          <AlertDialogBody>
            <VStack w="full" justifyContent="left" alignItems="left">
              ...
              {folderNotCreated && (
                <Text fontSize="sm" color="red">
                  Oops! Could not create folder. Please try again.
                </Text>
              )}
            </VStack>
          </AlertDialogBody>
