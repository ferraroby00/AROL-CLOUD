...
export default function RenamePrompt(props: RenamePromptProps) {
  ...
  const [newFileName, setNewFileName] = useState<string>(
    !renamePromptOpen?.isDir && renamePromptOpen?.name.endsWith(".pdf")
      ? renamePromptOpen?.name.slice(0, -4) || ""
      : renamePromptOpen?.name || "",
  );
  ...
  useEffect(() => {
    ...
    async function create() {
      ...
      try {
        ...
        if (!renamePromptOpen?.isDir && !documentUID) return;
          ...
          setFileMap((val) => {
            ...
            Object.entries(val).forEach(([key, value]) => {
              if (key === parentID) {
                ...
                if (childIndex > -1) newValue.childrenIds[childIndex] = newFileID;
  ...
  function handleRenameClicked() {
    const duplicate = Object.values(fileMap).find(
      (el: any) => el.parentId === currentFolderId && el.name === `${newFileName}.pdf`,
    );
  ...
  return (
    <AlertDialog isOpen={renamePromptOpen !== null} leastDestructiveRef={cancelRef} onClose={handleCancel}>
      <AlertDialogOverlay>
        <AlertDialogContent>
          ...
          <AlertDialogBody>
            <VStack w="full" justifyContent="left" alignItems="left">
              <Text fontSize="sm">
                Please type a new name for the Rename {renamePromptOpen?.isDir ? "folder" : "file"}
              </Text>
              <InputGroup>
                <Input
                  w="full"
                ...
                />
                {!renamePromptOpen?.isDir && <InputRightAddon>.pdf</InputRightAddon>}
              </InputGroup>
              {newFileName.length === 0 && (
                <Text fontSize="sm" color="red">
                  {renamePromptOpen?.isDir ? "Folder" : "File"} name cannot be empty
                </Text>
              )}
              {fileExists && (
                <Text fontSize="sm" color="red">
                  A {renamePromptOpen?.isDir ? "folder" : "file"} with this name already exists
                </Text>
              )}
              {fileNotRenamed && (
                <Text fontSize="sm" color="red">
                  Could not rename {renamePromptOpen?.isDir ? "folder" : "file"}. Please make sure you have the
          ...
          <AlertDialogFooter>
            ...
            <Button colorScheme="blue" isLoading={isRenaming} loadingText="Saving" onClick={handleRenameClicked} ml={3}>
              Rename
            </Button>
          </AlertDialogFooter>
        </AlertDialogContent>
