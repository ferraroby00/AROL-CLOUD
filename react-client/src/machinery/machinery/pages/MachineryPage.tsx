...
import SensorsPanel from "../../sensors/components/SensorsPanel";
...
export default function MachineryPage(props: DashboardProps) {
  ...
  useEffect(() => {
    ...
    async function getMachinery() {
      ...
      try {
        ...
        console.log(e);
        axiosExceptionHandler.handleAxiosExceptionWithToast(e, toast, "Machinery not found");
        navigate("/machineries");
        return;
      }
    ...
    getMachinery();
  }, [location.state, machineryUID, navigate, toast]);
  ...
  useEffect(() => {
    ...
    async function getData() {
      try {
        ...
        console.log(e);
        axiosExceptionHandler.handleAxiosExceptionWithToast(e, toast, "Could not load machinery sensors");
      }
    }

    getData();
  }, [machinery, machineryUID, toast]);
  ...
  return (
    <Box w={"full"}>
      {Number(principal?.companyID) === 0 ? (
        <Breadcrumb mb={2}>
          ...
          <BreadcrumbItem>
            <BreadcrumbLink
              onClick={() =>
                navigate("/companies/" + location.state.machinery.companyID, { state: "machinery_management" })
              }
            >
          ...
          <BreadcrumbItem isCurrentPage>
            <BreadcrumbLink>{machineryUID}</BreadcrumbLink>
          </BreadcrumbItem>
        </Breadcrumb>
      ) : (
        <Breadcrumb mb={6}>
          <BreadcrumbItem>
            <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "/machineries", false)}>Machineries</BreadcrumbLink>
          </BreadcrumbItem>
          ...
          {props.type === "dashboard" && (
            <BreadcrumbItem isCurrentPage={true}>
              <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "/machinery/" + machineryUID + "/dashboard", true)}>
                Dashboard
              </BreadcrumbLink>
            </BreadcrumbItem>
          )}
          {props.type === "documents" && (
            <BreadcrumbItem isCurrentPage={true}>
              <BreadcrumbLink onClick={(e) => breadcrumbNavigate(e, "/machinery/" + machineryUID + "/documents", true)}>
                Documents
              </BreadcrumbLink>
            </BreadcrumbItem>
          )}
      ...
      {!machineryLoading && machinery && machineryUID && (
        <>
          {Number(principal?.companyID) === 0 ? (
            <Heading mb={6}>
              {" "}
              {props.type === "dashboard" ? " Dashboard" : props.type === "documents" ? " Documents" : "Sensors"}
            </Heading>
          ) : (
          ...
          {props.type === "landing" && <LandingPanel machinery={machinery} />}
          {props.type === "dashboard" && (
            <DashboardPanel
              machinery={machinery}
              dashboard={dashboard}
          ...
          {props.type === "documents" && (
            <DocumentsPanel machinery={machinery} documentsPermissions={documentsPermissions} />
          )}
          {props.type === "sensors" && (
            <SensorsPanel machineryUID={machinery.uid}/>
          )}
        </>
      )}
      {machineryLoading && (
        <VStack w={"full"} h={"700px"} justifyContent={"center"} alignItems={"center"}>
          <Spinner size={"xl"} />
        </VStack>
      )}
    </Box>
