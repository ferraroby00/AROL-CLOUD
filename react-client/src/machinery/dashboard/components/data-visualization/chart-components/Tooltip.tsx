...
export default function ChartTooltip(props: ChartTooltipProps) {
  ...
  useEffect(() => {
    ...
    props.chartTooltipData.chartCoordinate,
    props.chartTooltipData.clickCoordinate,
    window.innerHeight,
    window.innerWidth,
  ]);
  ...
  const sensorsToDisplayValue = useMemo(() => getSensorsToDisplayValue(), [props.chartTooltipData.sensorData]);
  ...
  function getSensorsToDisplayValue() {
    ...
    if (Object.keys(payload.aggregationData).length === 0) {
      return Object.entries(payload.activeData)
        .map(([sensorInternalName, sensorValue]) => {
          let diff = "";
          if (props.chartTooltipData.sensorDataIndex > 0 && sensorValue !== null) {
            let prevIndex = props.chartTooltipData.sensorDataIndex - 1;
            if (
              props.chartTooltipData.displayData[prevIndex].allData.hasOwnProperty(sensorInternalName) &&
              props.chartTooltipData.displayData[prevIndex].allData[sensorInternalName] !== null
            ) {
              diff = (
                sensorValue - props.chartTooltipData.displayData[prevIndex].allData[sensorInternalName]!!
              ).toFixed(2);
            }
      ...
      return Object.entries(payload.aggregationData)
        .map(([aggregationName, aggregationValue]) => {
          let aggregationEntry = props.chartTooltipData.aggregationsArray.find(
            (el: Aggregation) => el.name === aggregationName,
          );
          let sensorEntry = props.chartTooltipData.sensorsMonitoringArray.find((el: SensorMonitoring) =>
            aggregationValue.note.endsWith(el.internalName),
          );
          ...
          if (props.chartTooltipData.sensorDataIndex > 0) {
            ...
            if (props.chartTooltipData.displayData[prevIndex].aggregationData.hasOwnProperty(aggregationName)) {
              diff = (
                ...
                props.chartTooltipData.displayData[prevIndex].aggregationData[aggregationName].value
              ).toFixed(2);
            }
          } else if (props.chartTooltipData.leftData.length > 0) {
            let prevSensorData = props.chartTooltipData.leftData.slice(-1);
            if (prevSensorData.length > 0 && prevSensorData[0].aggregationData.hasOwnProperty(aggregationName)) {
  ...
  function getDiff(diff: string) {
    ...
    if (diff === "0.0" || diff === "-0.0") {
      return (
        <Text fontWeight={400} fontSize={"sm"} pl={2}>
          No change
        </Text>
      );
      ...
      >
          ...
          <>
            <HStack w={"full"} justifyContent={"space-between"}>
              <Heading size={"sm"} whiteSpace={"nowrap"} w={"full"} textAlign={"center"}>
                {props.chartTooltipData.label}
              </Heading>
              <CloseButton size={"md"} onClick={handleCloseTooltip} />
            </HStack>
            <Divider orientation={"horizontal"} />
            <VStack maxH={"300px"} overflowY={"auto"} alignItems={"left"}>
              {sensorsToDisplayValue.map((sensorData: any, index: number) => (
                <HStack key={index} justifyContent={"space-between"}>
                  <HStack>
                    <Box boxSize={6} bgColor={sensorData.color} borderRadius={"md"} />
                  ...
                  <HStack alignItems={"space-between"}>
                    ...
                    <HStack w={"90px"} justifyContent={"right"}>
        ...
        {props.chartTooltipData.sensorData[0].payload.machineryOff && (
          <HStack alignItems={"flex-start"}>
            <VStack alignItems={"justifyContent"}>
              <Text fontSize={"md"} fontWeight={600}>
                Machinery OFF for {getHoursMachineryOff()}
              </Text>
              <Text fontSize={"sm"} mt={"0!important"} fontWeight={400}>
                From {dayjs(props.chartTooltipData.sensorData[0].payload.machineryOffFrom).format("D MMM YYYY HH:mm")}
              </Text>
              <Text fontSize={"sm"} mt={"0!important"} fontWeight={400}>
                To {dayjs(props.chartTooltipData.sensorData[0].payload.machineryOffTo).format("D MMM YYYY HH:mm")}
              </Text>
            </VStack>
            <CloseButton size={"md"} onClick={handleCloseTooltip} />
          </HStack>
