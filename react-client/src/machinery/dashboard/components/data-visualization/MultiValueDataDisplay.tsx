...
function MultiValueDataDisplay(props: MultiValueDataDisplay) {
  ...
  useEffect(() => {
    ...
    setChartProps((val) => {
      const allSensorData = [
        ...props.sensorData.leftData,
        ...props.sensorData.displayData,
        ...props.sensorData.rightData,
      ];

      const min = Math.min(
        ...allSensorData
          .filter((el) => !el.machineryOff)
          .map((el) => Math.min(...(Object.values(el.allData).filter(Number) as number[]))),
      );

      const max = Math.max(
        ...allSensorData
          .filter((el) => !el.machineryOff)
          .map((el) => Math.max(...(Object.values(el.allData).filter(Number) as number[]))),
      );
  ...
  useEffect(() => {
    ...
    setPolarChartSensorData((val) => {
      ...
      props.sensorsMonitoringArray.forEach((sensorMonitoring) => {
        let allDataMap: Map<string, number> = new Map();
        allSensorData.forEach((el) => {
          if (
            !el.activeData.hasOwnProperty(sensorMonitoring.internalName) ||
            el.activeData[sensorMonitoring.internalName] === null
          )
            return;
          let key = (~~(el.activeData[sensorMonitoring.internalName]!! / bucketSize)).toString();
          if (allDataMap.has(key)) {
            let occurrences = allDataMap.get(key)!!;
        ...
        let bucketArray: any[] = [];
        for (let [key, value] of Array.from(allDataMap.entries())) {
          bucketArray.push({
      ...
      props.aggregationsArray.forEach((aggregation) => {
        ...
        allSensorData.forEach((el) => {
          if (
            !el.aggregationData.hasOwnProperty(aggregation.name) ||
            el.aggregationData[aggregation.name].value === null
          )
            return;
          let key = (~~(el.aggregationData[aggregation.name].value!! / bucketSize)).toString();
          if (allDataMap.has(key)) {
            let occurrences = allDataMap.get(key)!!;
            allDataMap.set(key, occurrences + 1);
          } else {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.sensorsMonitoringArray, props.aggregationsArray, props.sensorData]);
  ...
  useEffect(() => {
    ...
    setPolarChartSensorData((val) => {
      let numSectionsNeeded = props.sensorsMonitoringArray.length + props.aggregationsArray.length;
      ...
      let resultingSectionSize = ~~(shortestDimension / numSectionsNeeded);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.dataDisplaySize, props.sensorsMonitoringArray, props.aggregationsArray]);

  //Chart ZOOM - +/- buttons
  useEffect(() => {
    if (!chartZoom.doZoom) return;

    let zoomIn = chartZoom.zoomAmount < 0;
    let zoomOut = chartZoom.zoomAmount > 0;

    let zoomAmount = Math.abs(chartZoom.zoomAmount);

    props.setSensorData((val) => {
      let labelIndex = chartZoom.pivotIndex;

      let xAxisMiddleIndex = ~~(val.displayData.length / 2);

      let displayDataLength = val.displayData.length;
      let rightDataLength = val.rightData.length;
      let leftDataLength = val.leftData.length;
      ...
      if (labelIndex < xAxisMiddleIndex) {
        if (zoomIn) {
          val.numSamplesDisplaying -= val.displayData
            .slice(displayDataLength - zoomAmount, displayDataLength)
            .filter((el) => !el.machineryOff).length;

          val.rightData = [
            ...val.rightData,
            ...val.displayData.slice(displayDataLength - zoomAmount, displayDataLength).reverse(),
          ];
          ...
          if (rightDataLength) {
            ...
            val.numSamplesDisplaying += val.rightData
              .slice(rightDataLength - zoomAmount, rightDataLength)
              .filter((el) => !el.machineryOff).length;

            val.displayData = [
              ...val.displayData,
              ...val.rightData.slice(rightDataLength - zoomAmount, rightDataLength).reverse(),
            ];
            ...
            val.numSamplesDisplaying += val.leftData
              .slice(leftDataLength - zoomAmount, leftDataLength)
              .filter((el) => !el.machineryOff).length;
  ...
  useEffect(() => {
    props.setSensorData((val) => {
      ...
      let panAmount = chartPan.panAmount;
      ...
      if (panAmount < 0 && rightDataLength > 0) {
        ...
        val.numSamplesDisplaying += val.rightData
          .slice(-moveAmount, rightDataLength)
          .filter((el) => !el.machineryOff).length;
        ...
        val.displayData = [
          ...val.displayData.slice(moveAmount, displayDataLength),
          ...val.rightData.slice(-moveAmount, rightDataLength).reverse(),
        ];
        ...
        val.numSamplesDisplaying += val.leftData
          .slice(leftDataLength - moveAmount, leftDataLength)
          .filter((el) => !el.machineryOff).length;
        val.numSamplesDisplaying -= val.displayData
          .slice(displayDataLength - moveAmount, displayDataLength)
          .slice(0, moveAmount)
          .filter((el) => !el.machineryOff).length;

        val.rightData = [
          ...val.rightData,
          ...val.displayData.slice(displayDataLength - moveAmount, displayDataLength).reverse(),
        ];
        val.displayData = [
          ...val.leftData.slice(leftDataLength - moveAmount, leftDataLength),
          ...val.displayData.slice(0, displayDataLength - moveAmount),
        ];
  ...
  useEffect(() => {
    ...
    props.setSensorData((val) => {
      let numSamplesToDisplay = props.sensorData.displayData.length;

      let allSensorData = [...val.leftData, ...val.displayData, ...val.rightData.reverse()];
      let selectedIndex = allSensorData.length - 1 - chartQuickNavigate;
  ...
  function handleChartMouseMove(a: any) {
    ...
    let tooltipIndex = a.activeTooltipIndex;
  ...
  function handleChartMouseUp(a: any) {
    ...
    if (chartZoomSelector.startIndex === chartZoomSelector.endIndex) {
      ...
      let ref = chartContainerRef!!.current!!;

      props.setChartTooltip({
        active: true,
        label: a.activeLabel,
        chartCoordinate: [
          ref.getBoundingClientRect().x + window.scrollX,
          ref.getBoundingClientRect().y + window.scrollY,
        ],
      ...
      setChartZoomSelector((val) => {
        ...
        if (numSamplesSelected < 5) {
          let numSamplesNeededLeft = ~~((5 - numSamplesSelected) / 2);
  ...
  function handleChartMouseLeave(a: any) {
  ...
  function legendNameTranslator(sensorName: string) {
    if (sensorName.startsWith("allData.")) {
      ...
      if (sensorName.startsWith("H")) {
        ...
        if (splittedSensorName.length !== 2) return "Unknown sensor";

        const foundSensor = props.availableSensors.find((el) => el.internalName === splittedSensorName[1]);

        if (!foundSensor) return "Unknown sensor";
  ...
  function navigateToNewData() {
    props.setSensorData((val) => {
      ...
      let numElementsNeeded = val.displayData.length - newDisplayData.length;
      ...
      if (numElementsNeeded < 0) {
        val.leftData = [
          ...val.leftData,
          ...val.displayData,
          ...val.rightData.slice(0, -1 * numElementsNeeded).reverse(),
        ];
      } else {
    ...
    <>
      ...
      >
          ...
          <Box>
            <ResponsiveContainer width={props.dataDisplaySize.width} height={props.dataDisplaySize.height}>
              <LineChart
                ...
                onMouseMove={(a) => handleChartMouseMove(a)}
                onMouseDown={(a) => handleChartMouseDown(a)}
                onMouseUp={(a) => handleChartMouseUp(a)}
                onMouseLeave={(a) => handleChartMouseLeave(a)}
              >
                <YAxis
                ...
                />
                <XAxis {...chartProps.xAxisProps} padding={{ left: 10, right: 10 }} />

                {!props.chartTooltipActive && <Tooltip content={<></>} />}
                <Tooltip trigger={"click"} content={<></>} />

                {props.displayType === "fullscreen" && (
                  <Legend iconType={"circle"} formatter={(value) => legendNameTranslator(value)} />
                )}
        ...
        {props.widget.type === "area-chart" && (
          <ResponsiveContainer width={props.dataDisplaySize.width} height={props.dataDisplaySize.height}>
            ...
            >
              ...
              {props.displayType === "fullscreen" && (
                <Legend iconType={"rect"} formatter={(value) => legendNameTranslator(value)} />
              )}
        ...
        {props.widget.type === "pie-chart" && (
          <>
            <ResponsiveContainer width={props.dataDisplaySize.width} height={props.dataDisplaySize.height - 40}>
              <PieChart>
                {/*<YAxis/>*/}
                <Tooltip content={(props) => <PieTooltip tooltipProps={props} />} />
                {props.sensorsMonitoringArray.map((sensor, index) => (
                  <Pie
                    ...
                    outerRadius={
                      (polarChartSensorData.sectionSize * index + (polarChartSensorData.sectionSize - 10)) / 2
                    }
            ...
            <HStack mt={"-8px!important"} mb={2} px={2} w={"full"} justifyContent={"space-between"}>
              <Text fontWeight={400} color={"gray.500"}>
                Starting from {polarChartSensorData.startingFromTime}
              </Text>
              <Button
                size={"sm"}
                variant={"outline"}
                ...
                loadingText={"Loading"}
                isDisabled={props.sensorData.endOfData}
                title={props.sensorData.endOfData ? "All sensor data already loaded" : ""}
                onClick={() => props.setLoadingMoreSensorData(true)}
              >
                Load previous
              </Button>
            </HStack>
          ...
          <>
            ...
            >
              <IconButton
                ...
                variant={"unstyled"}
                title={"Move left"}
                isDisabled={props.sensorData.leftData.length === 0}
                isLoading={props.sensorData.leftData.length === 0 && props.loadingMoreSensorData}
                icon={<FiChevronLeft />}
                onClick={(e) => handlePanChartButtonClicked(e, "pan-left")}
                aria-label={"Pan left"}
                _hover={{
              ...
              <HStack>
                <IconButton
                  ...
                  variant={"unstyled"}
                  title={"Zoom in"}
                  isDisabled={props.sensorData.displayData.length <= 5}
                  icon={<FiZoomIn />}
                  onClick={(e) => handleZoomChartButtonClicked(e, "zoom-in")}
                  aria-label={"Zoom in"}
                  _hover={{
                    bgColor: "rgb(255,255,255, 0.75)",
                  }}
                ...
                <IconButton
                  ...
                  variant={"unstyled"}
                  title={"Zoom out"}
                  isDisabled={props.sensorData.leftData.length === 0 && props.sensorData.rightData.length === 0}
                  icon={<FiZoomOut />}
                  onClick={(e) => handleZoomChartButtonClicked(e, "zoom-out")}
                  aria-label={"Zoom out"}
                  _hover={{
                    bgColor: "rgb(255,255,255, 0.75)",
                  }}
                ...
                {props.displayType === "dashboard" && (
                  <>
                    <IconButton
                      ...
                      isDisabled={props.sensorData.displayData.length <= 5}
                      icon={<FiMaximize />}
                      onClick={() => setChartFullscreenModalOpen(true)}
                    ...
                    <IconButton
                      ...
                      isDisabled={props.sensorData.displayData.length <= 5}
                      icon={<FiCalendar />}
                      onClick={() => setQuickNavigateModalOpen(true)}
              ...
              <IconButton
                ...
                variant={"unstyled"}
                title={"Move right"}
                isDisabled={props.sensorData.rightData.length === 0}
                icon={<FiChevronRight />}
                onClick={(e) => handlePanChartButtonClicked(e, "pan-right")}
            ...
            <HStack
              ...
              alignItems={"center"}
            >
              <Text w={"full"} textAlign={"left"} fontSize={"sm"} fontWeight={400}>
                {props.sensorData.displayData.length ? props.sensorData.displayData[0].formattedTime : ""}{" "}
                {props.sensorData.leftData.length === 0 ? "(end)" : ""}
              </Text>
              <Text w={"full"} textAlign={"center"} fontSize={"sm"} fontWeight={400}>
                {props.sensorData.numSamplesDisplaying} samples
              </Text>
              <Text w={"full"} textAlign={"right"} fontSize={"sm"} fontWeight={400}>
                {props.sensorData.displayData.length ? props.sensorData.displayData.slice(-1)[0].formattedTime : ""}{" "}
                {props.sensorData.rightData.length === 0 ? "(end)" : ""}
              </Text>
            </HStack>
            {props.sensorData.hasNewData && (
              ...
              >
                <HStack w={"full"} alignItems={"center"} justifyContent={"space-between"}>
                  <Text fontSize={"md"} fontWeight={650}>
                    Update
                  </Text>
      ...
      {props.displayType === "dashboard" && chartFullscreenModalOpen && (
