...
function SingleValueDataDisplay(props: SingleValueDataDisplayProps) {
  ...
  useEffect(() => {
    ...
    if (displayData.length > 0) {
      ...
      //Display AGGREGATE
      if (
        Object.entries(displayDataEntry.aggregationData).length > 0 &&
        ...
        let filteredLeftData = leftData.filter((val) => {
          if (Object.entries(val.aggregationData).length === 0 || !val.aggregationData.hasOwnProperty("aggregation"))
            return false;
        ...
        if (filteredLeftData.length > 0) {
          ...
          if (previousAggregation.aggregationData.aggregation.value) {
            valueDiff = (
              displayDataEntry.aggregationData.aggregation.value - previousAggregation.aggregationData.aggregation.value
            ).toFixed(2);
          }
        }
        ...
        if (objectKeys.length > 0 && displayDataEntry.allData[objectKeys[0]] !== null) {
          ...
          if (filteredLeftData.length > 0) {
            ...
            if (previousSampleObjectKeys.length > 0 && previousSample.allData[previousSampleObjectKeys[0]]) {
              valueDiff = (
                displayDataEntry.allData[objectKeys[0]]!! - previousSample.allData[previousSampleObjectKeys[0]]!!
              ).toFixed(2);
            }
          }
    ...
    //FALLBACK VALUES - if no entries in display data show last historical value
      ...
      if (Object.entries(leftDataEntry.aggregationData).length > 0 && leftDataEntry.aggregationData.aggregation.value) {
        if (
          Object.entries(leftDataEntry.aggregationData).length > 0 &&
      ...
      {sensorDataToDisplay && (
        <>
          {props.widget.type === "current-value" && (
            <VStack>
              <HStack w={"full"}>
                <Text fontSize={48}>{sensorDataToDisplay.value !== "" ? sensorDataToDisplay.value : "N/A"}</Text>
                <VStack alignItems={"left"}>
                  <Text fontSize={"sm"}>{props.sensorMonitoring.name}</Text>
                  <Text fontSize={"lg"} fontWeight={500} mt={"0!important"}>
                    {props.sensorMonitoring.unit}
                  </Text>
                </VStack>
                {sensorDataToDisplay.valueDiff && (
                  <>
                    ...
                    <VStack>
                      <Text fontSize={"sm"} fontWeight={300} mb={"0!important"}>
                        Difference
                      </Text>
                      <HStack alignItems={"center"} mt={"0!important"}>
                        {Number(sensorDataToDisplay.valueDiff) >= 0 && <FiArrowUp size={28} />}
                        {Number(sensorDataToDisplay.valueDiff) < 0 && <FiArrowDown size={28} />}
                        <Text h={"full"} fontSize={24}>
              ...
              <Text fontSize={"sm"} fontWeight={300} color={"gray.500"} mt={"0!important"}>
          ...
          {props.widget.type === "thermostat" && (
            <VStack h={"full"} w={"full"}>
              <HStack w={"full"} alignItems={"center"} onMouseDown={(e) => e.stopPropagation()}>
                <VStack flexGrow={1}>
                  <Thermometer
                    theme="light"
                ...
                <VStack alignItems={"center"} flexGrow={1}>
                  <Text fontSize={"sm"} textAlign={"center"}>
                    {props.sensorMonitoring.name}
                  </Text>
                  <Divider orientation={"horizontal"} />
                  <Text fontSize={40}>{sensorDataToDisplay ? sensorDataToDisplay.value : "N/A"}</Text>
                  <Text fontSize={"md"} fontWeight={500} mt={"0!important"}>
                    {props.sensorMonitoring.unit}
                  </Text>
                  {sensorDataToDisplay.valueDiff && (
                    <>
              ...
              <Text fontSize={"sm"} fontWeight={300} color={"gray.500"} pt={1}>
                {getSensorDataTimeOfSampling()}
              </Text>
            </VStack>
          )}
          {props.widget.type === "tachometer" && (
            <VStack>
              <GaugeChart
                ...
                arcWidth={0.3}
                percent={
                  isNaN(parseFloat(sensorDataToDisplay.value))
                    ? 0
                    : parseFloat(sensorDataToDisplay.value) / sensorDataToDisplay.maxValue
                }
              />
              <HStack w={"full"} justifyContent={"center"}>
                <Text fontSize={32}>{sensorDataToDisplay.value ? sensorDataToDisplay.value : "N/A"}</Text>
                <VStack alignItems={"left"}>
                ...
                {sensorDataToDisplay.valueDiff && (
                  <>
                    <Divider orientation={"vertical"} h={"55px"} />
                    <VStack>
                      <Text fontSize={"xs"} fontWeight={300} mb={"0!important"}>
                        Difference
                      </Text>
                      <HStack alignItems={"center"} mt={"0!important"}>
                        {Number(sensorDataToDisplay.valueDiff) >= 0 && <FiArrowUp size={28} />}
                        {Number(sensorDataToDisplay.valueDiff) < 0 && <FiArrowDown size={28} />}
                        <Text h={"full"} fontSize={16}>
                          {Number(sensorDataToDisplay.valueDiff) >= 0
                            ? "+" + sensorDataToDisplay.valueDiff
                            : sensorDataToDisplay.valueDiff}
                        </Text>
