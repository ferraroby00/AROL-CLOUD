...
export default function HistoryModal(props: HistoryModalProps) {
  ...
  useEffect(() => {
    ...
    let sensorFound = props.availableSensors.find((val) => val.internalName === sensorMonitoring);
  ...
  useEffect(() => {
    if (!sensorToDisplay) return;

    // console.log(props.sensorData.leftData.map((el)=>(el.formattedTime)))

    //Display data added to the end
    let sensorData = [...props.sensorData.displayData, ...[...props.sensorData.leftData].reverse()];

    setSensorDataToDisplay(
      sensorData.filter((val) => {
  ...
  return (
    <Modal isOpen={props.historyModalOpen} onClose={handleClose} size={"5xl"} scrollBehavior={"inside"}>
      ...
      <ModalContent onMouseDown={(e) => e.stopPropagation()}>
        ...
        <ModalBody>
          {sensorToDisplay && (
            <>
              <Text fontSize={"md"}>Showing sensor data history. The most recent samples are listed first.</Text>
                  ...
                  <Tbody>
                    {sensorDataToDisplay.map((sensorData, index) => {
                      ...
                      if (currValue && index + 1 < sensorDataToDisplay.length) {
                        let prevValue = sensorDataToDisplay[index + 1].allData[sensorToDisplay.internalName!!];
                        if (prevValue) {
                      ...
                      if (props.sensorsMonitoring.aggregations.length > 0) {
                        if (props.sensorsMonitoring.dataRange.unit === "sample") {
                          ...
                          if (
                            index + 1 >= sensorDataToDisplay.length ||
                            sensorDataToDisplay[index + 1].time < props.sensorData.minDisplayTime
                          ) {
                            aggregateDataSetEnded = true;
                          }
                        }
                          ...
                          <Tr>
                            ...
                            <Td textAlign={"center"}>
                              <HStack flexWrap={"nowrap"} justifyContent={"center"}>
                                <span>{diffValue}</span>
                                {diffValue && (parseFloat(diffValue) >= 0 ? <FiArrowUp /> : <FiArrowDown />)}
                              </HStack>
                            </Td>
                          </Tr>
                          {aggregateDataSetEnded && (
                            <Tr>
                              <Td colSpan={5} textAlign={"center"} fontWeight={500}>
                                End of aggregated data set
                              </Td>
                            </Tr>
                          )}
                        </Fragment>
              ...
              {!props.sensorData.endOfData && (
                ...
                >
                  ...
                  {loadingMoreSensorData && <Spinner size={"xl"} />}
                </HStack>
              )}
            </>
          )}
          {!sensorToDisplay && !sensorToDisplayNotFound && (
            <VStack w={"full"} h={"250px"} justifyContent={"center"} alignItems={"center"}>
              <Spinner size={"xl"} />
            </VStack>
          )}
          {sensorToDisplayNotFound && <Text fontSize={"md"}>Sensor history is unavailable. Please try again.</Text>}
        </ModalBody>
      </ModalContent>
    </Modal>
  );
