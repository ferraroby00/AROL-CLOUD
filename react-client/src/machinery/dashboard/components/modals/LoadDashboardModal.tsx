...
export default function LoadDashboardModal(props: LoadDashboardModalProps) {
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      try {
        let result = await dashboardService.getDashboards(props.machinery.uid);
        setSavedDashboards(result);
      } catch (e) {
        console.log(e);
        axiosExceptionHandler.handleAxiosExceptionWithToast(e, toast, "Dashboards could not be loaded");
      }
    ...
    getData();
  }, [props.machinery.uid, tabIndex, toast]);
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      try {
        let result = await dashboardService.getDashboardTemplates(props.machinery); // MODIFIED
        setTemplateDashboards(result);
      } catch (e) {
        console.log(e);
        axiosExceptionHandler.handleAxiosExceptionWithToast(e, toast, "Dashboard templates could not be loaded");
      }
  ...
  return (
    <Modal size={"lg"} isOpen={props.loadDashboardModalOpen} onClose={closeModal} scrollBehavior={"inside"}>
      ...
      <ModalContent onMouseDown={(e) => e.stopPropagation()}>
        ...
        <ModalBody pb={12}>
          ...
          >
            ...
            <TabPanels>
              <TabPanel>
                <Text fontSize={"md"} fontWeight={400} color={"gray.500"} textAlign={"center"} mb={5}>
                ...
                {loading && (
                  <VStack w={"full"} h={"150px"} justifyContent={"center"} alignItems={"center"}>
                    <Spinner size={"md"} />
                  </VStack>
                )}
              </TabPanel>
  ...
  return (
    <VStack w={"full"} justifyContent={"left"}>
      <HStack w={"full"} mt={2} justifyContent={"space-between"}>
        ...
        <VStack>
          <Button w={"full"} colorScheme="teal" variant="solid" onClick={handleLoadDashboardButton}>
            Load
          </Button>
          <Button
            w={"full"}
