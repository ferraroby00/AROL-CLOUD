...
export default function QuickNavigateModal(props: QuickNavigateModalProps) {
  ...
  useEffect(() => {
    ...
    let allSensorData: SensorData[] = [
      ...
      ...sensorData.displayData.reverse(),
      ...sensorData.leftData.reverse(),
    ];
  ...
  return (
    <Modal isOpen={props.quickNavigateModalOpen} onClose={handleClose} size={"md"} scrollBehavior={"inside"}>
      ...
      <ModalContent onMouseDown={(e) => e.stopPropagation()}>
        ...
        <ModalBody>
          <VStack w={"full"} alignItems={"center"}>
                ...
                {entry.show && (
                  <>
                    ...
                    >
                      {!entry.machineryOff && (
                        <>
                          <Text fontSize={"md"} color={selectedIndex === index ? "white" : "black"}>
                            {entry.formattedTime}
                          </Text>
                          <Text
                            fontSize={"sm"}
            ...
            {!props.sensorData.endOfData && (
              ...
              >
                {!props.loadingMoreSensorData && (
                  <HStack w={"full"} p={3} justifyContent={"center"} alignItems={"center"}>
                    <FiPlus />
            ...
            {dataToDisplay.length === 0 && <Text fontSize={"md"}>No sensor data available.</Text>}
          </VStack>
        </ModalBody>
        <ModalFooter>
          ...
          <Button colorScheme="blue" onClick={handleNavigateButtonClicked} isDisabled={selectedIndex < 0}>
            Navigate
          </Button>
        </ModalFooter>
      </ModalContent>
