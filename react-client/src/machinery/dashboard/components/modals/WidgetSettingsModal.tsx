...
export default function WidgetSettingsModal(props: WidgetSettingsModalProps) {
  ...
  function handleAggregationSelected(value: string, index: number) {
    setSensorsMonitoring((val) => {
      ...
      if (props.maxSelectableSensors > 1) {
        let hasAggregations = val.aggregations.filter((el) => el.name !== "none").length > 0;
  ...
  function reColorSensorsMonitoring(val: { [key: string]: SensorDataFilter[] }, hasAggregates: boolean) {
    let colorIndex = 0;
  ...
  function handleSaveButtonClicked() {
    ...
    ToastHelper.makeToast(toast, "Changes saved", "info");
  }
  ...
  return (
    <Modal
      isCentered
      size={"2xl"}
      onClose={() => props.setSettingsModalOpen(false)}
      ...
      <ModalContent onMouseDown={(e) => e.stopPropagation()}>
        ...
        <ModalBody>
          <VStack w={"full"} alignItems={"left"}>
            {props.maxSelectableSensors > 1 && (
              <VStack alignItems={"left"} pt={2}>
                ...
                <HStack w={"full"} mx={8} justifyContent={"center"}>
                  ...
                  />
                  <Select value={sensorsMonitoring.dataRange.unit} onChange={handleRangeUnitSelected} w={250}>
                    {rangeUnits.map((unit) => (
            ...
            {(props.maxSelectableSensors === 1 || sensorsMonitoring.aggregations.length > 0) && (
              <>
                <VStack w={"full"} alignItems={"left"}>
                  <Text fontSize={"lg"} fontWeight={500}>
                    Data aggregations
                    ...
                    <VStack key={index} w={"full"} alignItems={"left"}>
                      <HStack w={"full"}>
                        <Text fontSize={"md"} whiteSpace={"nowrap"}>
                          Aggregation {props.maxSelectableSensors === 1 ? "type" : index + 1}
                        </Text>
                        <Select value={value.name} onChange={(e) => handleAggregationSelected(e.target.value, index)}>
                          ...
                          {aggregationOptions
                            ...
                            .map((aggregationOption, indexOption) => (
                              <option key={index + "_" + indexOption} value={aggregationOption.value}>
                        ...
                        {props.maxSelectableSensors > 1 &&
                          ...
                          index < aggregationOptions.length - 1 && ( //show button only if not all aggregation entries are already applied (-2 since index starts from 0)
                            <Button
                              variant={"solid"}
                              colorScheme={"teal"}
                              onClick={() => handleRemoveAggregate(value.name, index)}
                            >
                              -
                            </Button>
                          )}
                      </HStack>
                      {props.maxSelectableSensors === 1 && value.name !== "none" && (
            ...
            <VStack alignItems={"left"} pt={2}>
              ...
              {areSensorsMonitored() &&
                Array.from(Object.entries(sensorsMonitoring.sensors)).map(([_, value]) =>
        ...
        <ModalFooter>
          <Button mr={3} onClick={() => props.setSettingsModalOpen(false)}>
            Discard changes
          </Button>
...
function SensorMonitoredEntry(props: SensorMonitoredEntryProps) {
  ...
  function handleRemoveMonitoredSensor() {
    props.setMonitoredSensors((val) => {
      let foundEntry = val.sensors[sensor.category].find((el) => el.headNumber === props.indexHead);
      if (foundEntry) {
      ...
      <VStack w={"full"}>
        <HStack w={"full"} mt={2} justifyContent={"space-between"}>
          <HStack>
            <VStack alignItems={"left"}>
              <HStack justifyContent={"left"} alignItems={"center"}>
                {props.maxSelectableSensors === 1 && <Box boxSize={6} bgColor={"black"} borderRadius={"md"} />}
                {props.maxSelectableSensors > 1 && <Box boxSize={6} bgColor={props.color} borderRadius={"md"} />}
          ...
          <HStack justifyContent={"right"}>
            <Button colorScheme={"red"} variant="outline" onClick={handleRemoveMonitoredSensor}>
              Stop monitoring
            </Button>
          </HStack>
        </HStack>
  ...
  return (
    <HStack w={"full"} alignItems={"top"} justifyContent={"space-between"} pl={6}>
      <HStack>
        <Flex>
          <Box boxSize={"125px"}>
            <Image objectFit="cover" boxSize="100%" src={require("../../../../assets/machineries/EQUA.png")} />
          </Box>
        </Flex>
