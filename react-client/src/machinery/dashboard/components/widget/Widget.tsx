...
function Widget(props: DashboardWidgetProps) {
    ...
    async function getData() {
      ...
      try {
        ...
        if (sensorsMonitoring.requestType === "first-time" && result.displaySensorData.length > 0) {
          ...
          let cachedData = await machineryService.getMachinerySensorsData(
            props.machinery.uid,
            sensorsMonitoringForCachedData,
          );
              ...
              if (props.widget.category === "single-value") {
                ...
                if (val.rightData.length === 0) {
                  val.numSamplesDisplaying -= val.displayData
                    .slice(0, result.newSensorData.length)
        ...
        console.log(e);
        axiosExceptionHandler.handleAxiosExceptionWithToast(e, toast, "Sensor data could not be loaded");
  ...
  useEffect(() => {
    ...
    props.dashboardSize.rowHeight,
    props.dashboardSize.width,
    props.dashboardSize.numCols,
    props.layout.h,
    props.layout.w,
  ]);
  ...
  }, [sensorsMonitoring, props.availableSensors, sensorsMonitoringArray, aggregationsArray]);
  ...
  return (
    <Fragment>
      ...
      >
        ...
        />
        {!sensorsDataLoading && numSensorsMonitoring === 0 && (
          <VStack w={"full"} h={dataDisplaySize.height} justifyContent={"center"} alignContent={"center"}>
            <Box
              p={3}
              _hover={{
                cursor: "pointer",
            ...
            >
              <Text textAlign={"center"} fontSize={50} color={"gray.500"}>
                +
              </Text>
              <Text textAlign={"center"} fontSize={"sm"} color={"gray.500"} mt={"-3!important"}>
                Choose sensors
              </Text>
            </Box>
          </VStack>
        ...
        {!sensorsDataLoading && numSensorsMonitoring > 0 && (
          <>
            <VStack w={"full"} justifyContent={"center"} alignContent={"center"}>
              {sensorData.displayData.length && sensorsMonitoringArray.length && (
                <Box w={"full"} h={dataDisplaySize.height} maxH={dataDisplaySize.height}>
              ...
              {!sensorDataError && sensorData.numSensorData === 0 && (
                <VStack w={"full"} h={dataDisplaySize.height} justifyContent={"center"} alignContent={"center"}>
            ...
            <HStack h={"auto"} w={"full"} justifyContent={"center"} textOverflow={"ellipsis"} whiteSpace={"nowrap"}>
              <Text
                mt={"-12px"}
                fontSize={"xs"}
                color={"gray.400"}
