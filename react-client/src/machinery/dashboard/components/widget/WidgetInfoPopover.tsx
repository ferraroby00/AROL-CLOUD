        ...
        >
          ...
          <PopoverBody>
            <VStack w={"full"} h={"full"} alignItems={"flex-start"}>
              ...
              <VStack w={"full"} maxH={300} overflowY={"auto"} alignItems={"left"}>
                {props.sensorsMonitoringAggregations.length > 0 && (
                  <>
                    ...
                    {props.sensorsMonitoringAggregations.map((aggregation) => (
                      <HStack key={aggregation.name} w={"full"} flexWrap={"nowrap"}>
                        <Box boxSize={6} bgColor={aggregation.color} borderRadius={"md"} />
                        <Text whiteSpace={"nowrap"} color={aggregation.color}>
                          {aggregation.name}
                        </Text>
                      </HStack>
                    ))}
                ...
                {Array.from(Object.entries(props.sensorsMonitoringSensors)).map(([_, value]) =>
                  value.map((headMechEntry) =>
                    headMechEntry.sensorNames.map((sensorName, index) => (
                      <HStack key={sensorName.name + "-" + headMechEntry.headNumber} w={"full"} flexWrap={"nowrap"}>
                        <Box boxSize={6} bgColor={sensorName.color} borderRadius={"md"} />
                        <Text whiteSpace={"nowrap"} color={sensorName.color}>
