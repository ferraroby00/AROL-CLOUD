...
import FilteringBarProps from "../classes/FilteringBarProps";
      ...
      >
        <HStack width={"100%"} flexWrap={"wrap"} justifyContent={"space-between"} gap={5}>
          ...
          <HStack ml={"0!important"} flexWrap={"nowrap"}>
            ...
            />
            <Button colorScheme={"blue"} size={"md"} onClick={handleSearchButtonPressed}>
              Search
            </Button>
          </HStack>
        </HStack>
...
function FiltersBar(props: FilteringBarProps) {
  function handleTopLevelFilterCheckboxChanged(filterKey: string, event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!props.machinerySortingAndFilters.filtersMap.get(filterKey)) return false;

    props.setMachinerySortingAndFilters((val: MachinerySortingAndFilters) => {
      val!!.filtersMap
        .get(filterKey)!!
        .filterEntries.forEach((filterValue: any) => (filterValue.selected = event.target.checked));

      let numEntriesAffected = val!!.filtersMap.get(filterKey)!!.filterEntries.length;
  ...
  function handleFilterCheckboxChanged(
    ...
    props.setMachinerySortingAndFilters((val: MachinerySortingAndFilters) => {
      val!!.filtersMap
        .get(filterKey)!!
        .filterEntries.find((value) => value.entryInternalName === filterValue)!!.selected = event.target.checked;
      return {
        sortingList: val.sortingList,
        filtersMap: new Map(val.filtersMap),
      };
  ...
  function controlIfAllFilterCheckboxesChecked(filterKey: string) {
    ...
    return (
      props.machinerySortingAndFilters!!.filtersMap.get(filterKey)!!.filterEntries.filter((val) => !val.selected)
        .length === 0
    );
  }
  ...
  function controlIfFilterCheckboxesAreIndeterminate(filterKey: string) {
    ...
    let numUnchecked = props
      .machinerySortingAndFilters!!.filtersMap.get(filterKey)!!
      .filterEntries.filter((val) => !val.selected).length;
    return (
      numUnchecked !== 0 &&
