...
import SortingAndPaginationBarProps from "../classes/SortingAndPaginationBarProps";
import Page from "../classes/Page";
import { ChevronUpDownIcon } from "@heroicons/react/24/solid";

export default function SortingAndPaginationBar(props: SortingAndPaginationBarProps) {
  let paginationEntries = [10, 25, 50];

  function getShownItemsRange(): string {
    let from = props.sortingAndPagination.currentPage * props.sortingAndPagination.pageSize;
  ...
  function handleSortDropdownChanged(event: React.ChangeEvent<HTMLSelectElement>) {
    let sortIndex = parseInt(event.target.value);
    props.setSortBy({
  ...
  function handlePageSizeDropdownChanged(event: React.ChangeEvent<HTMLSelectElement>) {
    props.setPage((val: Page) => ({
      ...
      skeletonArray: Array(parseInt(event.target.value))
        .fill(1)
        .map((x, y) => x + y),
    }));
  }
  ...
  return (
    ...
    >
      <Box>
        <Text>
          {getShownItemsRange()} out of {props.sortingAndPagination.totalItems}
        </Text>
      </Box>
      <HStack flexWrap={"nowrap"}>
        <Select
          ...
          width={"fit-content"}
          onChange={(e) => handleSortDropdownChanged(e)}
        ...
        <Select
          ...
          icon={<ChevronUpDownIcon />}
          width={"fit-content"}
          onChange={(e) => handlePageSizeDropdownChanged(e)}
          //value={props.machinerySortingAndFilters.sortingList.find((val)=>(val.selected))?.entry}
        >
