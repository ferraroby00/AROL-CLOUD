...
export default function LogsFilteringBar(props: LogsFilteringBarProps) {
  ...
  function handleDateChanged(from: boolean, to: boolean, event: React.ChangeEvent<HTMLInputElement>) {
    ...
    let inputDate = dayjs(event.target.value);
    console.log(inputDate);
    if (from) {
      if (props.machineryLogsFilter?.to && inputDate.isAfter(dayjs(props.machineryLogsFilter.to))) {
    ...
    if (to) {
      if (props.machineryLogsFilter?.from && inputDate.isBefore(dayjs(props.machineryLogsFilter.from))) {
  ...
  return (
    ...
    >
        ...
        <VStack>
          <InputGroup>
            ...
            <Input
              ...
              onChange={(e) => handleDateChanged(true, false, e)}
            />
          </InputGroup>
        ...
        <VStack>
          <InputGroup>
            ...
            <Input
              ...
              onChange={(e) => handleDateChanged(false, true, e)}
            />
          </InputGroup>
          {dateErrors.toError && (
            <Text size={"sm"} color={"red.500"}>
      ...
      <HStack ml={"0!important"}>
        <Button size={"md"} onClick={handleResetButtonPressed}>
          Reset
        </Button>
