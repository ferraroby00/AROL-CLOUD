...
const sidebarItems = [
  {
    ...
    icon: FiHome,
    link: "/",
    selectedMatcher: "home",
  },
  {
    name: "Machineries",
    icon: FiCodesandbox,
    link: "/machineries",
    selectedMatcher: "machineries",
  },
  {
    name: "Dashboard",
    icon: FiGrid,
    link: "/dashboards",
    selectedMatcher: "dashboards",
  },
  {
    name: "Documents",
    icon: FiFolder,
    link: "/documents",
    selectedMatcher: "documents",
  },
  {
    name: "Users management",
    icon: FiUsers,
    link: "/users",
    selectedMatcher: "users",
  },
  {
    name: "Machinery permissions",
    icon: FiLock,
    link: "/permissions",
    selectedMatcher: "permissions",
  },
  {
    name: "Machineries management",
    icon: FiCodesandbox,
    link: "/companies",
    selectedMatcher: "machinery_management",
  },
  {
    name: "Companies management",
    icon: FiSettings,
    link: "/companies",
    selectedMatcher: "company_management",
  },
  {
    name: "Machinery permissions",
    icon: FiLock,
    link: "/companies",
    selectedMatcher: "machinery_permissions",
  },
];
...
export default function Sidebar(props: any) {
  ...
  const displaySidebar = useMemo(() => sidebarStatus.type === "sidebar", [sidebarStatus.type]);

  const displayWidgetSelector = useMemo(() => sidebarStatus.type === "widget-selector", [sidebarStatus.type]);

  return (
    <>
      {displaySidebar && <SidebarContent onClose={() => props.onClose} display={"block"} />}
      {displayWidgetSelector && <WidgetSelectorContent onClose={() => props.onClose} display={"flex"} />}
      <Drawer
        autoFocus={false}
        isOpen={props.isOpen}
        placement="left"
  ...
  return (
    ...
    >
      ...
      {sidebarItems.map((sidebarItem) => (
        <SidebarItem
          key={sidebarItem.selectedMatcher}
...
const SidebarItem = (props: NavItemProps) => {
  ...
  // CHECK IF SIDEBAR ITEM HAS TO BE DISPLAYED
  useEffect(() => {
    if (!principal) {
      setIsSelected(false);
    ...
    setShowItem(permissionChecker.hasSidebarItemAccess(principal, props.selectedMatcher)!);
  }, [principal, props.selectedMatcher]);
  ...
  useEffect(() => {
    ...
    if (!location.state) {
      setIsSelected(props.selectedMatcher === "home" && location.pathname === "/");
    } else setIsSelected(location.state === props.selectedMatcher);
  }, [principal, location, props.selectedMatcher]);

  return (
    <RouterLink to={props.link} hidden={!showItem} state={props.selectedMatcher}>
  ...
  return (
    ...
    >
      <Box
        ...
        onClick={() => navigate("/")}
      >
        <Image objectFit="cover" src={ArolLogo} alt="Arol logo" />
        <CloseButton display={{ base: "flex", md: "none" }} onClick={onClose} />
      </Box>
      <Divider orientation={"horizontal"} my={"0!important"} />
      <Box
        position={"relative"}
        w={"full"}
        pt={4}
      ...
      >
        <Box style={{ direction: "ltr" }}>
          ...
          <Text fontSize={"sm"} fontWeight={400} color={"gray.500"} textAlign={"center"} mb={3}>
...
const WidgetSelectorItem = memo((props: WidgetSelectorItemProps) => {
  ...
  const WidgetPreviewComponent = useMemo(() => WidgetPreview(props.widget.type), [props.widget.type]);
    ...
    case "thermostat": {
      return (
        <HStack w={"full"} alignItems={"baseline"} justifyContent={"center"} pb={3}>
          <Thermometer theme="light" value="18" max="100" format="°C" size="large" height="200" tooltipValue={false} />
        </HStack>
      );
    }
    case "tachometer": {
