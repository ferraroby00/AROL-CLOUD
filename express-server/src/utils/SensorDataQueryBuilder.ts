  ...
  switch (sensorFilters.requestType) {
    case constants.REQUESTTYPE_FIRST: {
      switch (sensorFilters.dataRange.unit) {
        case constants.DATARANGE_SAMPLE: {
        ...
        case constants.DATARANGE_MONTH: {
    ...
    case constants.REQUESTTYPE_CACHE: {
      ...
      query +=
        "AND to_milliseconds(time) < " +
        sensorFilters.cacheDataRequestMaxTime +
        " ORDER BY time DESC LIMIT " +
        cacheNumSamplesLimit +
        " ";
...
function buildCheckEndOfSensorDataQuery(
  ...
  query +=
    "iot_thing IN (" + sensorNamesArray.join(",") + ") AND to_milliseconds(time) < " + lastSampleTime + " LIMIT 1";
