...
function parseQueryResult(
  response: TimestreamQuery.Types.QueryResponse,
  category: string,
  ...
  if (category === "plc") {
    requiredSensorsMap.set(
      0,
      sensorFilter[0].sensorNames.map((el) => el.name),
    );
    ...
    sensorFilter.forEach((sensorFilterEntry) => {
      requiredSensorsMap.set(
        sensorFilterEntry.headNumber,
        sensorFilterEntry.sensorNames.map((el) => el.name),
      );
    });
  } else if (category === "ns") {
    sensorFilter.forEach((sensorFilterEntry) => {
