...
function groupBySensorName(sensorData: SensorDataSample[], numSamplesRequired: number, machinerySensors: Sensor[]) {
  ...
  sensorData.forEach((entry: { name: string; value: number; time: number }) => {
      ...
      //Merging done by the corresponding merging strategy (min, max, sum, avg, majority...)
        ...
        switch (bucketingType) {
          ...
          case "majority": {
            ...
            let maxEl = value[startIndex].value,
              maxCount = 1;
...
function bucketVerticallyAndAggregateMultiValue(
  sensorDataArray: [string, { value: number; time: number }[]][],
  ...
  while (!stop) {
    ...
    sensorDataCursors.forEach((cursorValue, index) => {
      if (
        cursorValue < sensorDataArray[index][1].length &&
        ...
        if (sensorData.length) {
          if (sensorData[sensorData.length - 1].activeData.hasOwnProperty(sensorDataArray[index][0])) {
            sensorDataObject.fillerData[sensorDataArray[index][0]] =
              sensorData[sensorData.length - 1].activeData[sensorDataArray[index][0]];
            sensorDataObject.allData[sensorDataArray[index][0]] =
              sensorData[sensorData.length - 1].activeData[sensorDataArray[index][0]];
          } else {
            sensorDataObject.fillerData[sensorDataArray[index][0]] =
              sensorData[sensorData.length - 1].fillerData[sensorDataArray[index][0]];
            sensorDataObject.allData[sensorDataArray[index][0]] =
              sensorData[sensorData.length - 1].fillerData[sensorDataArray[index][0]];
          }
        } else {
          ...
          switch (aggregation.name) {
            case "Minimum": {
              aggregateValue = Math.min(
                ...Object.values(sensorDataObject.allData).map((val) => (val !== null ? val : Number.MAX_SAFE_INTEGER)),
              );
              let sensorDataEntry = Object.entries(sensorDataObject.activeData).find(
                ([, val]) => val === aggregateValue,
              );
              if (sensorDataEntry) {
            ...
            case "Maximum": {
              aggregateValue = Math.max(
                ...Object.values(sensorDataObject.allData).map((val) => (val !== null ? val : Number.MIN_SAFE_INTEGER)),
              );
              let sensorDataEntry = Object.entries(sensorDataObject.activeData).find(
                ([, val]) => val === aggregateValue,
              );
...
function aggregateSingleValue(sensorData: SensorData[], sensorFilters: SensorDataFilters, displayMinTime: number) {
  if (
    sensorData.length > 0 &&
    sensorFilters.widgetCategory === constants.WIDGETTYPE_SINGLE &&
    sensorFilters.aggregations.length > 0
  ) {
    ...
    if (sensorFilters.dataRange.unit === constants.DATARANGE_SAMPLE) {
      numSamplesToSlice =
        ...
        sensorData.slice(-sensorFilters.dataRange.amount).filter((el) => el.machineryOff).length;
    } else {
    ...
    switch (aggregation.name) {
      case "Minimum": {
        ...
        sensorData
          .slice(-numSamplesToSlice)
          .filter((val) => !val.machineryOff)
          .forEach((val) => {
            let objectKeys = Object.keys(val.allData);

            if (objectKeys.length > 0 && val.allData[objectKeys[0]] && val.allData[objectKeys[0]]!! < min) {
      ...
      case "Maximum": {
        ...
        sensorData
          ...
          .forEach((val) => {
            ...
            if (objectKeys.length > 0 && val.allData[objectKeys[0]] && val.allData[objectKeys[0]]!! > max) {
      ...
      case "Average": {
        ...
        sensorData
          ...
          .forEach((val) => {
            ...
            if (objectKeys.length > 0 && val.allData[objectKeys[0]]) {
...
function insertMachineryOffPadding(sensorData: SensorData[], sensorFilters: SensorDataFilters) {
  ...
  if (sensorFilters.requestType === constants.REQUESTTYPE_CACHE) {
    ...
    if (timeDiff > 60000) {
      return [
        ...sensorData,
        ...generateMachineryOffEntries(
          sensorFilters,
          sensorFilters.cacheDataRequestMaxTime,
          timeDiff,
          sensorData[sensorData.length - 1].time,
        ),
      ];
    }
    ...
    if (timeDiff > 60000) {
      return [
        ...generateMachineryOffEntries(
          ...
          sensorData[0].time,
          timeDiff,
          sensorFilters.newDataRequestMinTime,
        ),
        ...sensorData,
      ];
    }
...
function generateMachineryOffEntries(
  sensorFilters: SensorDataFilters,
  time: number,
  timeDiff: number,
  ...
  for (let i = 0; i < numPaddingEntriesToAdd; i++) {
    let emptySensorDataObject: SensorData = {
      ...
      formattedTime: dayjs(time - (timeDiff / (numPaddingEntriesToAdd + 1)) * (numPaddingEntriesToAdd - i)).format(
        "D MMM YYYY HH:mm",
      ),
      machineryOff: true,
